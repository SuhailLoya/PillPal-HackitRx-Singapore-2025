...
import { Mic } from "lucide-react"; // <-- Import microphone icon
...
const QueryButton = ({ languageSetting }: QueryButtonProps) => {
    ...
    return (
        <Button
            className="rounded-full h-20 w-20 text-3xl font-bold bg-yellow-700 hover:bg-orange-600 text-white flex items-center justify-center"
            onClick={record}
            disabled={loading} // disable while waiting for API
        >
            {loading ? "..." : <Mic className="!size-7" />}
        </Button>
    );
};
